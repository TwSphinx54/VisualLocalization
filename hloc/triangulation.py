...
from .utils.read_write_model import (read_cameras_binary, read_images_binary, CAMERA_MODEL_NAMES)
...
def create_db_from_model(empty_model, database_path):
    ...
    for i, camera in cameras.items():
        ...
        db.add_camera(model_id, camera.width, camera.height, camera.params, camera_id=i, prior_focal_length=True)
...
def import_matches(image_ids, database_path, pairs_path, matches_path, min_match_score=None,
...
def main(sfm_dir, empty_sfm_model, image_dir, pairs, features, matches, colmap_path='colmap',

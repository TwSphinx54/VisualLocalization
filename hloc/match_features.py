...
@torch.no_grad()
def main_db(conf, pairs, features, export_dir, exhaustive=False):
    logging.info('Matching local features with configuration:'
                 f'\n{pprint.pformat(conf)}')

    feature_path = Path(export_dir, features + '_db.h5')
...
@torch.no_grad()
def main_query(conf, pairs, features, export_dir, outputsR, exhaustive=False):
    logging.info('Matching local features with configuration:'
                 f'\n{pprint.pformat(conf)}')

    feature_path = Path(outputsR, features + '_queries.h5')
    feature_path_db = Path(export_dir, features + '_db.h5')

    assert feature_path.exists(), feature_path
    assert feature_path_db.exists(), feature_path_db
    feature_file = h5py.File(str(feature_path), 'r')
    feature_file_db = h5py.File(str(feature_path_db), 'r')
    ...
    match_name = f'{features}_{conf["output"]}_{pairs_name}'
    match_path = Path(outputsR, match_name + '.h5')
    match_file = h5py.File(str(match_path), 'a')
    ...
    for pair in tqdm(pair_list, smoothing=.1):
        ...
        data = {}
        feats0, feats1 = feature_file[name0], feature_file_db[name1]
        for k in feats1.keys():
            data[k + '0'] = feats0[k].__array__()
        for k in feats1.keys():
            data[k + '1'] = feats1[k].__array__()
...
if __name__ == '__main__':
    ...
    main_db(confs[args.conf], args.pairs, args.features, args.export_dir, exhaustive=args.exhaustive)
